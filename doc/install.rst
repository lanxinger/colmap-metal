...
.. note::
    ...
    **The backend is part of COLMAP's ABI.** These containers are data members of
    ...
    process must agree, including ``pycolmap`` and extensions linked to a separate
    COLMAP build. Rebuild them together when changing the backend; existing wheels
    may use the previous auto-selected backend. Query a build with ``colmap -h``,
    ``pycolmap.__hash_map_backend__`` or ``colmap::kHashMapBackend``.
