...
FlatHashSet<frame_t> PoseGraph::ComputeLargestConnectedFrameComponent(
    const Reconstruction& reconstruction, bool filter_unregistered) const {
  return LargestConnectedFrameComponent(reconstruction, filter_unregistered);
}
...
int PoseGraph::MarkConnectedComponents(const Reconstruction& reconstruction,
  ...
  const std::vector<FlatHashSet<frame_t>> components =
      ConnectedFrameComponents(reconstruction, /*filter_unregistered=*/false);
  ...
  for (const FlatHashSet<frame_t>& component : components) {
    if (static_cast<int>(component.size()) < min_num_images) {
      break;
    }
    for (const frame_t frame_id : component) {
      cluster_ids[frame_id] = num_components;
    }
    ++num_components;
  }
